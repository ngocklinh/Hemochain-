...
if (isset($_POST['user_id'])) {
    ...
    $stmt = $conn->prepare("SELECT user.name, avatar, phone, email
                    FROM database_officer, user 
                    WHERE database_officer.user_id = user.id
                    AND database_officer.user_id = ?");
    $stmt->bind_param("s", $user_id);
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($name, $avatar, $phone, $email);
    if ($stmt->num_rows() > 0) {
        $stmt->fetch();
        ...
        echo '<i class="fa-regular fa-pen-to-square edit-ic" onclick="openEditDatabaseOfficerForm()"></i>
